...
**osint-mcp** 透過[模型上下文協定](https://modelcontextprotocol.io)為您的 AI 代理提供跨 12 個資料來源的 37 個工具。代理平行查詢所有來源,關聯資料,識別風險,並呈現統一的情報圖像 &mdash; 在單一對話中完成。
...
現有的 OSINT 工具一次只提供一個來源的原始資料。osint-mcp 讓您的 AI 代理能夠**同時跨所有來源進行推理**。
...
<td><b>設定</b></td>
<td>安裝每個工具,管理每個配置</td>
<td><code>npx osint-mcp</code> &mdash; 一個命令,零配置</td>
</tr>
</tbody>
</table>
...
相同的 JSON 配置格式。將命令指向 `npx osint-mcp` 或您的本地安裝路徑。
...
| **osint-mcp** | **OSINT 與偵察** | **37 個工具、12 個來源** |
